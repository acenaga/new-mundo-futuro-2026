<x-layouts.public>
        ...
        <div class="relative mx-auto max-w-7xl px-6 py-24 lg:px-8 lg:py-32">
            <div class="grid items-center gap-12 lg:grid-cols-2">
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <p class="max-w-lg font-body text-base leading-relaxed lg:text-lg"
                        :class="isDark ? 'text-[#9999b3]' : 'text-[#4a4a6a]'">
                        Aprende las arquitecturas más avanzadas y las herramientas que definirán la próxima década de la web. De WebGL a AI-Driven Interfaces.
                    </p>

                    <div class="flex flex-wrap items-center gap-4">
                        <a href="#cursos"
                            class="rounded-lg bg-[#f4bf27] px-6 py-3 font-display text-sm font-bold text-[#342600] transition-all hover:brightness-110">
                            EXPLORAR CURSOS →
                        </a>
                        <a href="#tutoriales"
                            class="rounded-lg border px-6 py-3 font-display text-sm font-semibold transition-colors"
                            :class="isDark
                ...
                <div class="flex items-center justify-center lg:justify-end">
                    <div class="relative">
                        ...
                        <img src="{{ asset('assets/img/logo-wb.png') }}" alt="Mundo Futuro"
            ...
            <div class="grid gap-4 lg:grid-cols-3">
                ...
                <div class="clip-hex-corner relative col-span-1 flex min-h-56 flex-col justify-end overflow-hidden rounded-xl p-6 lg:col-span-2"
                    ...
                    <div class="pointer-events-none absolute inset-0 bg-gradient-to-t"
                        :class="isDark ? 'from-[#0d0d18]/80 via-transparent' : 'from-[#eaeaf5]/90 via-transparent'"></div>
                    <div class="relative z-10 flex flex-col gap-3">
                        <div class="flex flex-wrap gap-2">
                            <span class="rounded-md bg-[#f4bf27] px-2 py-0.5 font-display text-xs font-bold text-[#342600]">
                                BEST SELLER
                            </span>
                            <span class="rounded-md px-2 py-0.5 font-display text-xs font-bold"
    ...
    <section id="tutoriales" class="py-20 transition-colors lg:py-28"
        ...
        <div class="mx-auto max-w-7xl px-6 lg:px-8">
            ...
            <div class="mb-10 text-center">
                ...
                <h2 class="mt-2 font-display text-3xl font-bold tracking-tight lg:text-4xl"
                    :class="isDark ? 'text-[#e2e2f0]' : 'text-[#12121d]'">
                                ...
                                @if ($tutorial->tags->isNotEmpty())
                                    <div class="flex flex-wrap gap-1">
                                        @foreach ($tutorial->tags->take(2) as $tag)
                                            <span class="rounded-md px-2 py-0.5 font-body text-xs"
        ...
        <div class="grid gap-16 lg:grid-cols-5">
            ...
            <div class="flex flex-col gap-6 lg:col-span-2">
                ...
                <p class="font-body text-sm leading-relaxed" :class="isDark ? 'text-[#9999b3]' : 'text-[#4a4a6a]'">
                    Perspectivas profundas sobre el ecosistema tecnológico, entrevistas con líderes de la industria y análisis de tendencias.
                </p>
                        ...
                        @foreach ($recentPosts as $i => $post)
                            <li class="flex items-start gap-3">
                                <span class="shrink-0 font-display text-xs font-bold tabular-nums"
                                    :class="isDark ? 'text-[#3a3a55]' : 'text-gray-300'">
                    ...
                    <article class="grid items-start gap-6 sm:grid-cols-2">
                        <div class="flex flex-col gap-3">
                            <div class="flex items-center gap-2 font-display text-xs"
                                ...
                                <span>{{ max(1, (int) ceil(str_word_count(strip_tags($post->body ?? '')) / 200)) }} min lectura</span>
                            </div>
                            ...
                            <p class="line-clamp-3 font-body text-sm leading-relaxed"
                                :class="isDark ? 'text-[#9999b3]' : 'text-[#4a4a6a]'">
                                {{ $post->excerpt }}
                            </p>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    <div class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full font-display text-xs font-bold"
                                        :class="isDark ? 'bg-[#3b2068] text-[#d3bbff]' : 'bg-[#e8e8ff] text-[#110090]'">
    ...
    <section class="transition-colors" :class="isDark ? 'bg-[#1b1b25]' : 'bg-[#eaeaf5]'">
        <div class="mx-auto max-w-2xl px-6 py-20 text-center lg:px-8">
            ...
            <p class="mx-auto mt-4 max-w-md font-body text-sm leading-relaxed"
                :class="isDark ? 'text-[#9999b3]' : 'text-[#4a4a6a]'">
                Suscríbete para recibir actualizaciones semanales sobre nuevas tecnologías, tutoriales exclusivos y ofertas en cursos.
            </p>

            <form class="mt-8 flex flex-col items-center gap-3 sm:flex-row" onsubmit="return false;">
                <input type="email" placeholder="Tu correo electrónico"
                    class="w-full flex-1 rounded-lg px-4 py-3 font-body text-sm outline-none ring-2 ring-transparent transition-all focus:ring-[#f4bf27]"
                    :class="isDark
                        ? 'bg-[#21212d] text-[#e2e2f0] placeholder:text-[#9999b3]'
                        : 'bg-white text-[#12121d] placeholder:text-gray-400 shadow-sm'">
                <button type="submit"
                    class="w-full rounded-lg bg-[#f4bf27] px-6 py-3 font-display text-sm font-bold text-[#342600] transition-all hover:brightness-110 sm:w-auto">
                    UNIRME AHORA
                </button>
            </form>

            <p class="mt-4 font-body text-xs" :class="isDark ? 'text-[#9999b3]/60' : 'text-gray-400'">
